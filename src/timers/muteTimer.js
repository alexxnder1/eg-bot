...
module.exports = {
    execute() {
        // 5 sec
        setInterval(() => {
            muteModel.findOne({ mutedStatus: true }, (err, res) => {
                    ...
                    guild.members.fetch().then((members) => {
                        members.forEach((member) => {
                            if(member.user.id == res.mutedId) {
                                ...
                                muteModel.findOneAndUpdate({ mutedId: res.mutedId, mutedStatus: true }, { $set: { unmutedReason: 'time expired', mutedStatus: false, unmutedBy: Client.user.id } }).then((err, res) => {
                                    if(err) return console.log(err);
                                });
